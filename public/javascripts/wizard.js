...
function setupUserData() {
  ...
  if (userData.citizen_id !== undefined) {
    document.getElementById("kycId").value = userData.citizen_id
  }
...
function submitKyc() {
  ...
  let idDOM = document.getElementById('kycId')
  ...
  setDisable([btnDOM, firstNameDOM, lastNameDOM, countryDOM, idDOM, addressDOM, pic1DOM, pic2DOM, pic3DOM, estimateDOM])
  const first_name = firstNameDOM.value
  const last_name = lastNameDOM.value
  const country = countryDOM.value
  const citizen_id = idDOM.value
  ...
  if (citizen_id == '' || citizen_id == undefined) { $('#kycIdAlert').addClass('invalid'); validate = false }
  if (address == '' || address == undefined) { $('#kycAddressAlert').addClass('invalid'); validate = false }
  if ((pic1 == '' || pic1 == undefined) && pic1Url === undefined) { $('#kycPic1Alert').addClass('invalid'); validate = false }
  ...
  if (validate === false) {
    setEnable([btnDOM, firstNameDOM, lastNameDOM, countryDOM, idDOM, addressDOM, pic1DOM, pic2DOM, pic3DOM, estimateDOM])
  } else {
    let uid = firebase.auth().currentUser.uid
    firebase.firestore().collection('users').doc(uid).update({
...
$(document).ready(function () {
  document.getElementById('kycFirstName').onkeydown = function() { $('#kycFirstNameAlert').removeClass("invalid") }
  document.getElementById('kycLastName').onkeydown = function() { $('#kycLastNameAlert').removeClass("invalid") }
  document.getElementById('kycId').onkeydown = function() { $('#kycIdAlert').removeClass("invalid") }
  document.getElementById('kycAddress').onkeydown = function() { $('#kycAddressAlert').removeClass("invalid") }
